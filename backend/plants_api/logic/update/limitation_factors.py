...
"""
from sqlalchemy import bindparam, exists, func, insert, select, text
...
async def insert_limitation_factors(
    ...
    statement = (
        ...
        .values(geometry=func.ST_SetSRID(func.ST_GeomFromText(bindparam("geometry_wkt"), text("4326"))))
        .returning(limitation_factor_parts.c.id)
    )
    ids = list(
        await conn.execute(
            statement,
            [{"limitation_factor_id": lf.id, "geometry_wkt": lf.geometry.wkt} for lf in limitation_factors_list],
        )
    )
