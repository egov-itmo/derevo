...
async def get_global_territory(conn: AsyncConnection) -> GlobalTerritory:
    ...
    lf_gdf = await greenlet_spawn(
        lambda: gpd.read_postgis(
            ...
            .join(limitation_factor_parts, limitation_factors.c.id == limitation_factor_parts.c.limitation_factor_id),
            conn.sync_engine,
            geom_col="geometry",
        )
    )
    ...
    ht_gdf = await greenlet_spawn(
        lambda: gpd.read_postgis(
            ...
            .join(humidity_type_parts, humidity_types.c.id == humidity_type_parts.c.humidity_type_id),
            conn.sync_engine,
            geom_col="geometry",
        )
    )
    terr_gdf = await greenlet_spawn(
        lambda: gpd.read_postgis(
            ...
            .join(soil_acidity_types, soil_acidity_types.c.id == territories.c.acidity_type_id)
            .join(soil_fertility_types, soil_fertility_types.c.id == territories.c.fertility_type_id),
            conn.sync_engine,
            geom_col="geometry",
        )
    )
    ...
    global_territory = GlobalTerritory(
        ...
        lt_gdf,
        ht_gdf,
        terr_gdf[["soil_type", "geometry"]].rename({"soil_type": "name"}, axis=1),
        terr_gdf[["acidity_type", "geometry"]].rename({"acidity_type": "name"}, axis=1),
        terr_gdf[["fertility_type", "geometry"]].rename({"fertility_type": "name"}, axis=1),
    )
