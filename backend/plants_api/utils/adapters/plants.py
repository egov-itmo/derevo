...
async def plant_dto_to_compositioner_plant(  # pylint: disable=too-many-locals,too-many-statements
    ...
    limitation_factors_statement = select(
        ...
        .scalar_subquery(),
        plants_limitation_factors.c.is_stable,
    ...
    humidity_statement = select(
        ...
        .scalar_subquery(),
        plants_humidity_types.c.is_stable,
    ...
    light_statement = select(
        ...
        plants_light_types.c.is_stable,
    ...
    soil_acidity_statement = select(
        ...
        .scalar_subquery(),
        plants_soil_acidity_types.c.is_stable,
    ...
    soil_fertility_statement = select(
        ...
        .scalar_subquery(),
        plants_soil_fertility_types.c.is_stable,
    ...
    for plant in plants:
        ...
        try:
            ...
            limitation_factors_resistances = {
                EnumAdapters.limitation_factors.get(lf): c_enum.ToleranceType.from_value(int(value))
                for lf, value in res
            }
            if None in limitation_factors_resistances:
                logger.warning("Some of the limitation factors was not found in mapping for plant with id={}", plant.id)
                del limitation_factors_resistances[None]

            res = list(await conn.execute(humidity_statement, payload))
            humidity_preferences = {
                EnumAdapters.humidity.get(ht): c_enum.ToleranceType.from_value(int(value)) for ht, value in res
            }
            if None in humidity_preferences:
                logger.warning("Some of the humidity types was not found in mapping for plant with id={}", plant.id)
                del humidity_preferences[None]

            res = list(await conn.execute(light_statement, payload))
            light_preferences = {
                EnumAdapters.light.get(lt): c_enum.ToleranceType.from_value(int(value)) for lt, value in res
            }
            if None in light_preferences:
                logger.warning("Some of the light types was not found in mapping for plant with id={}", plant.id)
                del light_preferences[None]

            res = list(await conn.execute(soil_acidity_statement, payload))
            soil_acidity_preferences = {
                EnumAdapters.acidity.get(lt): c_enum.ToleranceType.from_value(int(value)) for lt, value in res
            }
            if None in soil_acidity_preferences:
                logger.warning("Some of the soil acidity types was not found in mapping for plant with id={}", plant.id)
                del soil_acidity_preferences[None]

            res = list(await conn.execute(soil_fertility_statement, payload))
            soil_fertility_preferences = {
                EnumAdapters.fertility.get(lt): c_enum.ToleranceType.from_value(int(value)) for lt, value in res
            }
            if None in soil_fertility_preferences:
                logger.warning(
                    "Some of the soil fertility types was not found in mapping for plant with id={}", plant.id
                )
                del soil_fertility_preferences[None]

            res = list(await conn.execute(soil_type_statement, payload))
            soil_type_preferences = {
                EnumAdapters.soil.get(lt): c_enum.ToleranceType.from_value(int(value)) for lt, value in res
            }
            if None in soil_type_preferences:
                logger.warning("Some of the soil  types was not found in mapping for plant with id={}", plant.id)
                del soil_type_preferences[None]

            res = list(await conn.execute(usda_zone_statement, payload))
            usda_zone_preferences = {
                c_enum.UsdaZone.from_value(uz): c_enum.ToleranceType.from_value(int(value)) for uz, value in res
            }
            if None in usda_zone_preferences:
                logger.warning("Some of the usda zones was not found in mapping for plant with id={}", plant.id)
            ...
            logger.error("Could not transform PlantDTO with id={} to compositioner.Plant: {!r}", plant.id, exc)
