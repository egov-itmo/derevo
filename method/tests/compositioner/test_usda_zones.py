...
@pytest.fixture
def plants_with_all_usda_zones() -> list[Plant]:
    ...
    for i, usda_zone in enumerate(d_enum.UsdaZone):
        plants.append(
            Plant(
                ...
                usda_zone_preferences={usda_zone: d_enum.ToleranceType.POSITIVE},
            )
        )
    return plants
...
def test_error_no_plants(plants_with_all_usda_zones: list[Plant], territory_info: Territory):
    """USDA zones blocks each plant from being planted, so no compositions are available."""

    territory_info.usda_zone = [d_enum.UsdaZone.USDA2]
    compositions = get_compositions(
        plants_available=list(
            plant
            for plant in plants_with_all_usda_zones
            if plant.usda_zone_preferences[d_enum.UsdaZone.USDA2] != d_enum.ToleranceType.POSITIVE
        ),
...
@pytest.mark.xfail(reason="Fixing")
def test_no_plants_unacceptable_usda_zones(plants_with_all_usda_zones: list[Plant], territory_info: Territory):
    ...
    for usda_zone in list(d_enum.UsdaZone):
        ...
        compositions = get_compositions(
            plants_available=plants_with_all_usda_zones,
            cohabitation_attributes=[],
            territory=territory_info,
        )
        assert (
        ...
        if len(compositions) != 0:
            for plant in compositions[0]:
                try:
                    assert not any(
                        plant.usda_zone_preferences[usda] == d_enum.ToleranceType.NEGATIVE
                        for usda in territory_info.usda_zone
