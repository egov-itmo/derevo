...
def make_grid(polygon, edge_size, polygon_id):
    ...
    result_limsult = gpd.GeoDataFrame(gpd.GeoSeries(squaresult_lims[squaresult_lims.intersects(polygon)]), geometry=0, crs=32636)
    result_limsult = gpd.GeoDataFrame(result_limsult.intersection(polygon), geometry=0, crs=32636).rename(columns={0:'geometry'})
    result_limsult['id'] = polygon_id

    return result_limsult[['id', 'geometry']]
...
def ids(grid_id):
    ...
    new_lim_ids = list(filter(lambda i: not(type(i) is int), new_lim_ids))
    new_lim_ids = [v for l in new_lim_ids for v in l]
    grid_id = grid_id[~grid_id['lim_ids'].isin(new_lim_ids)]

    return grid_id

def green_lims(limitations, greenery_polygons):
    ...
    lim_over = lim_over.drop_duplicates(subset='sorted').drop('sorted',axis=1).sort_values('id_1').reset_index(drop=True)

    whole = lim_over[(lim_over['id_1']) == (lim_over['id_2'])]
    lim_over = lim_over[~lim_over.index.isin(whole.index)].reset_index(drop=True)
    lim_over['lim_ids'] = [int(x) if x == y else ','.join(sorted([str(int(x)), str(int(y))])) for x,y in list(zip(lim_over['limitation_factor_id_1'], lim_over['limitation_factor_id_2']))]
    ...
    parks_lims = parks_lims.groupby(['grid_id']).apply(lambda x: ids(x)).reset_index(drop=True)
